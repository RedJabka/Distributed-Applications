...
int cond_init(CNDTN * self, int id, int child_num, int trans, int *descriptors){
	self->c_id = id;
	self->child_num = child_num;
...
int req_init(Req * self, int id, int time){
	self->id = id;
	self->time = time;
...
int receive_and_check_quiet(CNDTN *condition, local_id from, MessageType type) {
	Message recv;
	int status = receive(condition, from, &recv);

	if ((status) || recv.s_header.s_magic != MESSAGE_MAGIC || recv.s_header.s_type != type) {
		return 1;
	}
